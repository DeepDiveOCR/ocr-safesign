...
def get_region_prefix(region_name):
    ...
    print("📌 get_region_prefix 함수 실행됨")
        ...
        for item in stanregin:
            if 'row' in item:
                for entry in item['row']:
                    # 🔍 모든 entry의 지역명 출력
                    print(f"🔄 entry 지역명: {entry.get('locatadd_nm')}", flush=True)
                    print(f"🔍 비교 대상 region_name: {region_name}", flush=True)

                    if entry.get('locatadd_nm') == region_name:
                        print(f"✅ 일치하는 지역명 발견: {entry.get('locatadd_nm')}", flush=True)
...
def get_trade_api_url(building_type):
    ...
    if api_type:
        return f"http://apis.data.go.kr/1613000/RTMSDataSvc{api_type}/getRTMSDataSvc{api_type}"
    return None
...
def load_coords_by_building_type(building_type):
    ...
    filename = file_mapping.get(building_type)
    if not filename:
        raise ValueError(f"Unknown building_type: {building_type}")

    # real-estate-analyzer/estimator/final.py → ../../로 올라가서 ocr-safesign 루트로 접근
...
#######추가 
    if not deal_list:
        print("❌ 인근 단지 거래 데이터 없음 (deal_list is empty)")
        return None  # 또는 예외 발생 시킬 수도 있음
...
def get_estimated_price(address, building_type):
    region, dong, jibun = parse_address(address)
    lawd_cd = get_region_prefix(region)
    df_coords = load_coords_by_building_type(building_type)
    fallback_callback = lambda: fallback_strategy(address, lawd_cd, df_coords, building_type)
    ...
    if price is None:
        print("❌ 시세 추정 실패: 최종 가격이 None입니다.")
        return None, "데이터 없음"
    print(f"📌 [get_estimated_price] 입력 주소: {address}", flush=True)
    print(f"📌 [get_estimated_price] 건물 유형: {building_type}", flush=True)
    print(f"📊 최종 예측 가격: {price}", flush=True)
    print(f"📏 단가 기준 (per 평 or ㎡): {per}", flush=True)
    return price, per
