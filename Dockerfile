...
# 3. 환경 변수 설정: 파이썬이 버퍼링 없이 로그를 바로 출력하도록 하여,
# Firebase App Hosting 로그에서 실시간으로 상황을 볼 수 있게 합니다.
ENV PYTHONUNBUFFERED True

# 4. 작업 디렉토리 설정: 컨테이너(가상 컴퓨터) 내부에 /app 이라는 폴더를 만들고,
# 앞으로의 모든 작업은 이 폴더 안에서 이루어지도록 설정합니다.
WORKDIR /app

# 5. 의존성 파일 복사 및 설치: 먼저 requirements.txt만 복사하여
# 필요한 라이브러리들을 설치합니다. 이 과정을 분리하면 나중에 코드만 수정되었을 때
# 매번 모든 라이브러리를 새로 설치하지 않아 빌드 속도가 크게 향상됩니다.
COPY requirements.txt requirements.txt
RUN pip install --no-cache-dir -r requirements.txt

# 6. 소스 코드 복사: 현재 폴더(.)의 모든 파일과 폴더를
# 컨테이너의 /app 폴더 안으로 복사합니다.
COPY . .

# 7. 실행 명령어: gunicorn을 사용해 안정적인 운영 환경에서 Flask 앱을 실행합니다.
